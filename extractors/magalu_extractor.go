package extractors

import (
	"bytes"
	"encoding/json"
	"fmt"
	"io"
	"net/http"
	"net/url"
	"os"
	"os/exec"
	"path/filepath"
	"strconv"
	"strings"

	"github.com/google/uuid"
	"muambr-api/models"
	"muambr-api/utils"
)

// MagaluExtractor implements product extraction from Magazine Luiza
type MagaluExtractor struct {
	countryCode models.Country
}

// NewMagaluExtractor creates a new Magazine Luiza extractor
func NewMagaluExtractor() *MagaluExtractor {
	return &MagaluExtractor{
		countryCode: models.CountryBrazil,
	}
}

// GetCountryCode returns the country this extractor targets
func (e *MagaluExtractor) GetCountryCode() models.Country {
	return e.countryCode
}

// GetMacroRegion returns the macro region this extractor supports
func (e *MagaluExtractor) GetMacroRegion() models.MacroRegion {
	return e.countryCode.GetMacroRegion()
}

// GetIdentifier returns a static string identifier for this extractor
func (e *MagaluExtractor) GetIdentifier() string {
	return "magalu"
}

// BaseURL returns the base URL for Magazine Luiza searches
func (e *MagaluExtractor) BaseURL() string {
	return "https://www.magazineluiza.com.br"
}

// GetComparisons extracts product comparisons for a given search term
func (e *MagaluExtractor) GetComparisons(searchTerm string) ([]models.ProductComparison, error) {
	utils.Info("Starting Magazine Luiza extraction",
		utils.String("search_term", searchTerm),
		utils.String("extractor", "magalu"))

	// Build the search URL - Magazine Luiza uses /busca/ endpoint
	searchURL := fmt.Sprintf("%s/busca/%s/", e.BaseURL(), url.QueryEscape(searchTerm))
	
	utils.Info("Built Magazine Luiza search URL", 
		utils.String("url", searchURL),
		utils.String("extractor", "magalu"))

	// Fetch HTML content from Magazine Luiza
	htmlContent, err := e.fetchHTML(searchURL)
	if err != nil {
		utils.LogError("Failed to fetch HTML from Magazine Luiza", 
			utils.String("url", searchURL),
			utils.String("extractor", "magalu"),
			utils.Error(err))
		return nil, err
	}

	utils.Info("Successfully fetched HTML content from Magazine Luiza",
		utils.String("url", searchURL),
		utils.String("extractor", "magalu"),
		utils.Int("content_size_bytes", len(htmlContent)))

	// Extract products using Python script
	comparisons, err := e.extractWithPython(htmlContent)
	if err != nil {
		utils.LogError("Failed to extract products from Magazine Luiza with Python", 
			utils.String("extractor", "magalu"),
			utils.Error(err))
		return nil, err
	}

	utils.Info("Successfully extracted products from Magazine Luiza",
		utils.String("extractor", "magalu"),
		utils.Int("products_count", len(comparisons)))

	return comparisons, nil
}

// fetchHTML fetches HTML content from the given URL
func (e *MagaluExtractor) fetchHTML(url string) (string, error) {
	utils.Info("Starting HTTP request to Magazine Luiza",
		utils.String("url", url),
		utils.String("extractor", "magalu"),
		utils.String("base_domain", e.BaseURL()))

	// Create HTTP client with timeout
	client := &http.Client{}

	// Create request with proper headers to avoid being blocked
	req, err := http.NewRequest("GET", url, nil)
	if err != nil {
		return "", err
	}

	// Set headers to mimic a real browser
	req.Header.Set("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36")
	req.Header.Set("Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8")
	req.Header.Set("Accept-Language", "pt-BR,pt;q=0.8,en;q=0.5,en-US;q=0.3")
	req.Header.Set("Accept-Encoding", "gzip, deflate, br")
	req.Header.Set("DNT", "1")
	req.Header.Set("Connection", "keep-alive")
	req.Header.Set("Upgrade-Insecure-Requests", "1")

	resp, err := client.Do(req)
	if err != nil {
		utils.LogError("HTTP request failed for Magazine Luiza", 
			utils.String("url", url),
			utils.String("extractor", "magalu"),
			utils.Error(err))
		return "", err
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		utils.Warn("HTTP request returned non-200 status code from Magazine Luiza - possible anti-bot protection",
			utils.String("url", url),
			utils.String("extractor", "magalu"),
			utils.String("base_domain", e.BaseURL()),
			utils.Int("status_code", resp.StatusCode),
			utils.String("status", resp.Status),
			utils.String("user_agent", req.Header.Get("User-Agent")))
		return "", fmt.Errorf("HTTP request failed with status: %d", resp.StatusCode)
	}

	utils.Info("Successfully received HTTP response from Magazine Luiza",
		utils.String("url", url),
		utils.String("extractor", "magalu"),
		utils.Int("status_code", resp.StatusCode),
		utils.Any("content_length", resp.ContentLength))

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		utils.LogError("Failed to read response body from Magazine Luiza", 
			utils.String("url", url),
			utils.String("extractor", "magalu"),
			utils.Error(err))
		return "", err
	}

	utils.Info("Successfully fetched HTML content from Magazine Luiza",
		utils.String("url", url),
		utils.String("extractor", "magalu"),
		utils.Int("content_size_bytes", len(body)))

	return string(body), nil
}

// extractWithPython uses Python script to extract products from HTML
func (e *MagaluExtractor) extractWithPython(htmlContent string) ([]models.ProductComparison, error) {
	utils.Info("Starting Python extraction for Magazine Luiza",
		utils.String("extractor", "magalu"),
		utils.Int("html_size_bytes", len(htmlContent)))

	// Get absolute path to the Python script
	scriptPath := filepath.Join("extractors", "pythonExtractors", "magalu_page.py")
	if _, err := os.Stat(scriptPath); os.IsNotExist(err) {
		utils.LogError("Failed to get Python script path for Magazine Luiza",
			utils.String("script_path", scriptPath),
			utils.String("extractor", "magalu"),
			utils.Error(err))
		return nil, fmt.Errorf("Python script not found: %s", scriptPath)
	}

	// Get Python path from environment or use default
	pythonPath := os.Getenv("PYTHON_PATH")
	if pythonPath == "" {
		// Check for virtual environment
		if venvPath := os.Getenv("VIRTUAL_ENV"); venvPath != "" {
			pythonPath = filepath.Join(venvPath, "bin", "python3")
		} else {
			pythonPath = "python3" // Default for production environments like Render
		}
	}
	
	utils.Info("Using Python interpreter for Magazine Luiza extraction",
		utils.String("extractor", "magalu"),
		utils.String("python_path", pythonPath),
		utils.String("script_path", scriptPath))

	// Create temporary file for HTML content (to avoid argument length limits)
	tempFile, err := os.CreateTemp("", "magalu_html_*.html")
	if err != nil {
		utils.LogError("Failed to create temporary file for Magazine Luiza HTML",
			utils.String("extractor", "magalu"),
			utils.Error(err))
		return nil, fmt.Errorf("failed to create temp file: %w", err)
	}
	defer os.Remove(tempFile.Name()) // Clean up temp file
	defer tempFile.Close()

	// Write HTML content to temporary file
	if _, err := tempFile.WriteString(htmlContent); err != nil {
		utils.LogError("Failed to write HTML to temporary file for Magazine Luiza",
			utils.String("extractor", "magalu"),
			utils.String("temp_file", tempFile.Name()),
			utils.Error(err))
		return nil, fmt.Errorf("failed to write HTML to temp file: %w", err)
	}
	tempFile.Close() // Close file so Python can read it

	utils.Info("Created temporary HTML file for Magazine Luiza extraction",
		utils.String("extractor", "magalu"),
		utils.String("temp_file", tempFile.Name()))

	// Get current working directory for local packages
	workDir, _ := os.Getwd()
	
	// Prepare the Python command using the temporary file
	cmd := exec.Command(pythonPath, "-c", fmt.Sprintf(`
import sys
sys.path.append('%s')
sys.path.append('%s')
try:
    from magalu_page import extract_magalu_products
    import json
    
    with open('%s', 'r', encoding='utf-8') as f:
        html_content = f.read()
    products = extract_magalu_products(html_content)
    print(json.dumps(products))
except ImportError as e:
    print('{"error": "Missing Python dependencies", "details": "' + str(e) + '"}')
except Exception as e:
    print('{"error": "Python extraction failed", "details": "' + str(e) + '"}')
`, filepath.Dir(scriptPath), filepath.Join(workDir, "python_packages"), tempFile.Name()))

	// Execute the Python script
	var out bytes.Buffer
	var stderr bytes.Buffer
	cmd.Stdout = &out
	cmd.Stderr = &stderr

	utils.Info("Executing Python extraction script for Magazine Luiza",
		utils.String("extractor", "magalu"),
		utils.String("temp_file", tempFile.Name()))

	err = cmd.Run()
	if err != nil {
		utils.LogError("Python script execution failed for Magazine Luiza",
			utils.String("extractor", "magalu"),
			utils.String("python_path", pythonPath),
			utils.String("stderr", stderr.String()),
			utils.String("stdout", out.String()),
			utils.Error(err))
		return nil, fmt.Errorf("Python script execution failed: %w, stderr: %s", err, stderr.String())
	}

	utils.Info("Python script executed successfully for Magazine Luiza",
		utils.String("extractor", "magalu"),
		utils.String("python_stdout", out.String()),
		utils.String("python_stderr", stderr.String()))

	// Check if Python output contains error messages
	outputStr := out.String()
	if strings.Contains(outputStr, `"error":`) {
		var errorResp map[string]interface{}
		if json.Unmarshal(out.Bytes(), &errorResp) == nil {
			if errorMsg, ok := errorResp["error"].(string); ok {
				utils.Warn("Python script returned error for Magazine Luiza",
					utils.String("extractor", "magalu"),
					utils.String("error_type", errorMsg),
					utils.String("error_details", utils.GetString(errorResp["details"])))
				return []models.ProductComparison{}, nil // Return empty results instead of failing
			}
		}
	}

	// Parse the JSON output from Python
	var pythonProducts []map[string]interface{}
	err = json.Unmarshal(out.Bytes(), &pythonProducts)
	if err != nil {
		utils.LogError("Failed to parse Python JSON output for Magazine Luiza",
			utils.String("extractor", "magalu"),
			utils.String("python_output", out.String()),
			utils.Error(err))
		return nil, fmt.Errorf("failed to parse Python output: %w", err)
	}

	utils.Info("Successfully parsed Python output for Magazine Luiza",
		utils.String("extractor", "magalu"),
		utils.Int("products_found", len(pythonProducts)))

	// Convert to ProductComparison structs
	var comparisons []models.ProductComparison
	for _, product := range pythonProducts {
		// Get currency with fallback to extractor's country currency
		currency := utils.GetString(product["currency"])
		if currency == "" {
			currency = e.countryCode.GetCurrencyCode()
		}
		
		// Parse price as float64
		priceStr := utils.GetString(product["price"])
		price, err := strconv.ParseFloat(priceStr, 64)
		if err != nil {
			// Skip invalid price entries
			continue
		}

		// Generate unique UUID
		id := uuid.New().String()
		
		// Extract store URL safely
		storeURL := utils.GetString(product["url"])
		var storeURLPtr *string
		if storeURL != "" {
			storeURLPtr = &storeURL
		}
		
		comparison := models.ProductComparison{
			ID:          id,
			ProductName: utils.GetString(product["name"]),
			Price:       price,
			Currency:    currency,
			StoreName:   utils.GetString(product["store"]),
			StoreURL:    storeURLPtr,
			Country:     string(e.countryCode),
		}
		
		comparisons = append(comparisons, comparison)
	}

	utils.Info("Successfully converted products to comparisons for Magazine Luiza",
		utils.String("extractor", "magalu"),
		utils.Int("comparisons_count", len(comparisons)))

	return comparisons, nil
}